...
export default function StudentExamsPage() {
  ...
  const { data: entries, isPending: scheduleLoading } = useQuery({
    queryKey: ['exam-schedule', 'me', 'student', examId],
    queryFn: () => fetchMyExamSchedule(examId!, 'student'),
  ...
  const handleDownload = async () => {
    ...
    setDownloading(true);
    try {
      const blob = await downloadMyHallTicketPdf(examId, 'student');
      downloadBlob(blob, 'hall-ticket.pdf');
    } finally {
  ...
  return (
    <PageContainer>
      <PageHeader
        ...
        subtitle="Your upcoming exam schedule and hall ticket."
        action={
          !examsLoading &&
          exams &&
          exams.length > 0 && (
